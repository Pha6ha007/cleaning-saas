...
### 1.6.1 — 2026-02-05
- NEW: documented Google Maps JavaScript API as the primary map provider for Locations.
- NEW: documented Google Places API usage for address autocomplete with coordinate resolution.
- CHANGED: source of truth for Location coordinates is now Google Maps (autocomplete selection or draggable marker).
- FIXED: clarified that `latitude` / `longitude` are considered valid only after explicit user action.
- DEPRECATED: Leaflet / OpenStreetMap usage for Location coordinates.

---

### 1.6.0 — 2026-02-05
- CHANGED: clarified external dependency constraints for Google Maps & Places usage.
- FIXED: documented enforced API key limits (HTTP referrer restriction and allowed Google APIs only).

---

### 1.4.0 — 2026-02-04
- NEW: documented `is_active` flag on Location and its operational semantics.
- NEW: established deactivation (`is_active = false`) as the only supported way to remove a location from operational flows.
- CHANGED: creating jobs on inactive locations returns `400 Bad Request` with code `location_inactive`.
- CHANGED: deleting locations referenced by jobs returns `400 Bad Request` with code `location_has_jobs`.
- FIXED: documented Location → Job relationship as a hard invariant across history, reports, and PDFs.
- BREAKING: clients must handle `location_inactive` and `location_has_jobs` errors and migrate to deactivate/reactivate flow.
- DEPRECATED: physical deletion of locations in operational scenarios.

---

### 1.2.0 — 2026-02-04
- NEW: `GET /api/manager/jobs/export/` — CSV export of completed jobs for audit and reporting.
  - Period filter: `from` / `to` (based on `actual_end_time`).
  - Optional filters: `location_id`, `cleaner_id`, `sla_status`.
  - Export scope limited to manager’s company.
  - Response format: `text/csv` (valid CSV even for empty result sets).
- FIXED: CSV export uses the same business filters and semantics as `/api/manager/jobs/history/`.

---

### 1.1.0 — 2026-02-04
- NEW: `/api/manager/jobs/active/` — added flags `has_before_photo`, `has_after_photo`.
- CHANGED: `/api/manager/jobs/{id}/` — added fields `manager_notes`, `cleaner_notes`.
- FIXED: clarified SLA helper semantics (`compute_sla_status_for_job`, `compute_sla_reasons_for_job`).
